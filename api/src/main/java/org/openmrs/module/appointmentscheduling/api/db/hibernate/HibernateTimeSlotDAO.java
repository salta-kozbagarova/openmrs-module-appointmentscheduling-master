...
public class HibernateTimeSlotDAO extends HibernateSingleClassDAO implements TimeSlotDAO {
	...
	public List<TimeSlot> getTimeSlotsBySoftConstraints(AppointmentType appointmentType, Date fromDate, Date toDate,
		...
		if (fromDate != null && toDate != null && !fromDate.before(toDate)) {
			...
			String stringQuery = "SELECT timeSlot FROM TimeSlot AS timeSlot WHERE voided = 0 AND endDate > :startDate";
			...
			if (appointmentType != null) {
				stringQuery += " AND timeSlot.appointmentBlock IN("
			        + " FROM AppointmentBlock WHERE :appointmentType IN elements(types))";
            }
