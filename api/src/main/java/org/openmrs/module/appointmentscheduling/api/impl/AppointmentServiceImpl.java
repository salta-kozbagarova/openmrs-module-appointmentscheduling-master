...
//				//return only empty slots
//				if (getCountOfAppointmentsInTimeSlotThatAreNotCancelled(slot) == 0) {
...
//	@Override
//	@Transactional(readOnly = true)
//	public List<TimeSlot> getEmptyTimeSlotsByConstraints(AppointmentType appointmentType, Date fromDate, Date toDate, Provider provider, Location location, Patient excludeTimeSlotsWithPatient) throws APIException {
//
//		List<TimeSlot> suitableTimeSlots = getTimeSlotsByConstraintsIncludingFull(
//				appointmentType, fromDate, toDate, provider, location, excludeTimeSlotsWithPatient);
//
...
//	@Override
//	public List<TimeSlot> getTimeSlotsByConstraints(AppointmentType appointmentType, Date fromDate, Date toDate, Provider provider, Location location, Patient excludeTimeSlotsWithPatient, Boolean onlyEmpty) throws APIException {
//
//		List<TimeSlot> suitableTimeSlots = getTimeSlotsByConstraintsIncludingFull(
//				appointmentType, fromDate, toDate, provider, location, excludeTimeSlotsWithPatient);
//
...
//	}
	...
	public Set<PaymentType> getAllPaymentTypes() {
		HashSet set = new HashSet();
		set.addAll(getPaymentTypeDAO().getAll());
		return set;
	}

	@Override
	@Transactional(readOnly = true)
	public List<PaymentType> getAllPaymentTypes(boolean includeRetired) {
		return getPaymentTypeDAO().getAll(includeRetired);
	}

	@Override
	@Transactional(readOnly = true)
	public PaymentType getPaymentType(Integer paymentTypeId) {
		return (PaymentType) getPaymentTypeDAO().getById(paymentTypeId);
	}

	@Override
	@Transactional(readOnly = true)
	public PaymentType getPaymentTypeByUuid(String uuid) {
		return (PaymentType) getPaymentTypeDAO().getByUuid(uuid);
	}

	@Override
	@Transactional(readOnly = true)
	public List<PaymentType> getPaymentTypes(String fuzzySearchPhrase) {
		return getPaymentTypes(fuzzySearchPhrase, true);
	}

	@Override
	@Transactional(readOnly = true)
	public List<PaymentType> getPaymentTypes(String fuzzySearchPhrase, boolean includeRetired) {
		return getPaymentTypeDAO().getPaymentTypes(fuzzySearchPhrase, includeRetired);
	}

	@Override
	@Transactional
	public PaymentType savePaymentType(PaymentType paymentType) throws APIException {
		ValidateUtil.validate(paymentType);
		return (PaymentType) getPaymentTypeDAO().saveOrUpdate(paymentType);
	}

	@Override
	@Transactional
	public PaymentType retirePaymentType(PaymentType paymentType, String reason) {
		return savePaymentType(paymentType);
	}

	@Override
	@Transactional
	public PaymentType unretirePaymentType(PaymentType paymentType) {
		return savePaymentType(paymentType);
	}

	@Override
	@Transactional
	public void purgePaymentType(PaymentType paymentType) {
		getPaymentTypeDAO().delete(paymentType);
	}
	
	@Override
	@Transactional(readOnly = true)
	public List<PaymentType> getAllPaymentTypesSorted(boolean includeRetired) {
		List<PaymentType> paymentTypes = Context.getService(
				AppointmentService.class)
				.getAllPaymentTypes(includeRetired);
		Collections.sort(paymentTypes, new Comparator<PaymentType>() {

			public int compare(PaymentType pt1, PaymentType pt2) {
				return pt1.getName().toLowerCase()
						.compareTo(pt2.getName().toLowerCase());
			}
		});
		return paymentTypes;
	}
